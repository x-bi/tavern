...
关于长期记忆：酒馆 `PromptBuilderService` 永远不含 `conversation_memory`。长期记忆仅属于独立 AI 角色形态，按 `Companion.id` 隔离、默认关闭、显式开启；它不能写回酒馆角色卡、Persona、世界书或酒馆会话（见 [§17](#17-ai-角色形态分支)）。
...
- `prompts`：Prompt 预览 API（消费 `PromptBuilderService`）。
...
- `services/prompt-builder`：`PromptBuilderService`，酒馆形态唯一 Prompt 构建入口。
...
- Prompt 组装只能存在于 `services/prompt-builder` 相关模块。
...
- 会话流式生成期间需要会话级并发保护（`ChatService.conversationTasks`），避免同一会话同时写入多条 assistant 回复。
...
`PromptBuilderService` 是酒馆形态后端唯一 Prompt 构建入口。Prompt 预览 API（`prompts` 模块）与聊天 API（`chat` 模块）必须复用同一条 Builder 逻辑，不允许另写一套预览逻辑。

构建管线（在 `build()` 主流程中按硬编码顺序逐个 `addSection()` 创建，非插件式）：

1. 平台基础规则（`platform`）。
2. 角色卡（`character`）。
3. 用户 Persona（`persona`，如配置）。
4. 参数预设（`prompt_preset`，如配置）。
5. 输出规则（`output_rules`）。
6. 世界书命中条目（`worldbook`，按 before_history / after_history / before_current_user_input / after_current_user_input 四个插入点）。
7. 最近历史消息（`history`，按条数 + 字符上限裁剪）。
8. 当前用户输入（`current_user_input`）。
...
- `services/prompt-builder`：Prompt 构建和预览（基础设施服务）。
- `services/model-gateway`：模型供应商适配（基础设施服务）。
- `chat`：聊天编排和流式接口。
- `backups`：导入导出、备份恢复。
- `settings`：本地设置。
- `content-packs`：内容包导入。
...
AI 角色形态模块（见 [§17](#17-ai-角色形态分支)）：`companions`、`companion-messages`、`companion-memory`，以及独立的 AI 角色 Prompt Builder 与聊天路由。
...
阶段路线以 `model-context/stages` 为参考，执行时必须保持依赖关系。下表为酒馆 MVP 规划路线（当前主体已实现）：
...
当前进度：酒馆角色对话 MVP 主体已实现——角色、模型配置、会话消息、Prompt Builder、Model Gateway、SSE 聊天、停止 / 重新生成 / 消息操作、世界书、导入导出、备份恢复均落地。独立 AI 角色首版闭环也已实现：独立数据模型、持续消息线程、独立 Prompt Preview / SSE 聊天、可选长期记忆、版本恢复与 stale 重建、`/companion` 前端入口均已落地，现处于真实模型和长期使用验收阶段。
