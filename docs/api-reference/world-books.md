...
- **说明**：创建世界书；指定 `characterId` 校验角色归属。
- **请求体**（`CreateWorldBookDto`）：

  | 字段 | 类型 | 必填 | 校验规则 + 注释 |
  |---|---|---|---|
  | name | string | 是 | `@MaxLength(120)` |
  | characterId | string \| null | 否 | 关联角色 ID；null 表示全局 |
...
- **说明**：导入世界书 JSON（两阶段）。`formatVersion` 须为 `tavern-lite.world-book.v1`。旧命名 `before/after_current_user_message` 会归一化为 `before/after_current_user_input` 并产生 `INSERTION_ORDER_ALIAS_NORMALIZED` warning。JSON 非法/格式不符/角色不存在返回 400；同名冲突且策略 reject 返回 409（details 含 `suggestedName`）。
...
- **响应**：`{ fileName, template: { formatVersion, name, description, characterId, isEnabled, scanDepth, tokenBudget, metadata, entries: [...] } }`
...
- **路径参数**：`id: string`
- **说明**：部分更新世界书；`characterId` 传入（含 null）时校验角色归属。
