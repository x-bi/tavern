...
第 29 阶段先冻结 Model Gateway 抽象，当前不接入任何具体 provider。第 30 阶段再接入首个 OpenAI-compatible Chat Completions adapter。
...
- 共享无密钥契约：`packages/shared/src/model-gateway.ts`
- 后端网关模块：`apps/server/src/services/model-gateway`
- 业务模块只注入 `ModelGatewayService`，不得直接调用供应商 SDK、`fetch(providerUrl)` 或拼接供应商路径。
- 具体 provider 通过 `ModelGatewayRegistry.register(adapter)` 注册，adapter 必须实现 `testConnection`、`chat`、`streamChat`。

标准流事件：

- `delta`：增量文本，包含 `text`。
- `done`：完成事件，包含标准化 `ModelGatewayChatResult`。
- `error`：统一错误，包含稳定 `code` 和安全 `message`。
- `ping`：可选心跳，包含 ISO 时间。
