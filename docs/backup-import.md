...
- 核心记录有必要字段和可解析时间。
- 记录 ID、预设名、Persona 名、设置 `scope/key` 没有重复。
- 会话、消息、世界书、头像资源等引用能在备份文件内找到。

通过校验后，后端在一个 Prisma transaction 内执行：

1. 删除当前用户现有核心数据。
2. 按备份 ID 重建资源记录、角色、预设、Persona、会话、消息、世界书、世界书条目和设置。
3. 任一写入失败时事务回滚，避免只恢复一半。

## 安全策略

备份文件不会恢复模型供应商、模型、模型链或 API Key。恢复完成后，用户需要在模型页重新配置模型链。
