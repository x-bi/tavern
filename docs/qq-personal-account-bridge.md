...
- QQ 登录数据使用独立挂载目录；管理员退出时只清理该目录，不触碰 Tavern 数据库和上传文件。
...
需要更换 QQ 时，在“QQ 已接入”区域点击“退出并切换 QQ”并确认。系统会退出 NapCat 当前账号、清理该实例的持久化登录态并等待生成新二维码。原 QQ 接入记录、好友绑定和聊天历史都会保留，但旧账号会暂停同步；重新扫描原 QQ 时会自动恢复该账号。
